import { Log as Interface } from "@vendor";
import Settings from "@core/settings";
import { Logger } from "tslog";

export default class Log implements Interface {
    private _log: Logger<this>;
    ...
    constructor(settings?: Settings | null) {
        ...
        this._log = new Logger({
            ...
            prettyLogTemplate:
                "{{yyyy}}-{{mm}}-{{dd}}T{{hh}}:{{MM}}:{{ss}}.{{ms}}\t[{{name}}][{{logLevelName}}][{{filePathWithLine}}] ",
            prettyLogStyles: {
                logLevelName: {
    ...
    info(data: { message: string; extra?: object | null }) {
        ...
        if (data.extra) args.push(data.extra);
    ...
    error(data: {
        ...
        if (data.extra) args.unshift(data.extra);
